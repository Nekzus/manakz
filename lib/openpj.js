...
async function openProject(options) {
  ...
  if (!yesToAll) {
    ...
    const selectAll = await confirm(
      `${customEmoji.flash} Select 'Yes' for all options? (y/n)`
    );
    ...
    if (selectAll) {
      ...
      if (
        await confirm(
          `${customEmoji.file} Open project in Visual Studio Code? (y/n)`
        )
      ) {
        await openInVSCode();
      }
      ...
      if (
        await confirm(
          `${customEmoji.rocket} Start the development server? (y/n)`
        )
      ) {
        await startDevServer();
      }
