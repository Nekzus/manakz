...
async function installOrUpdateDependencies(autoExecute = false) {
  ...
  const shouldUpdateDeps = hasNodeModules && confirm(`${customEmoji.update} Do you want to update dependencies? (y/n)`)
  const shouldInstallDeps =
    !hasNodeModules && confirm(`${customEmoji.install} Do you want to install dependencies? (y/n)`)
